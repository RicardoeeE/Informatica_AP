...
def main():
    ...
    for step in range(n_steps):
        ...
        rueda_i = deepcopy(rueda_delantera_izquierda)
        tl = translacion([1.3, -0.1, 1.2])
        tf_ruedas = (tl @ Rz @ Ry @ Rx) @ tf_cabina
        rueda_i.transform(tf_ruedas)
        ...
        # Aplica el giro de -90 grados a todas las piezas
        cab.transform(tf_cubo)
        rueda_d.transform(tf_cubo)
        rueda_i2.transform(tf_cubo)
        rueda_d2.transform(tf_cubo)
        cubo.transform(tf_cubo)
